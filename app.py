...
# 8. [AJUSTE PARTE 1] Sincronizadas datas do formulário e filtro para corrigir contagem de vagas.
# 9. [AJUSTE PARTE 1] Limpeza do placeholder de pacotes ao trocar cliente.
...
def handle_selecao_cliente():
    ...
    # [AJUSTE PARTE 1] Limpa a mensagem de pacote anterior antes de processar
    placeholder = st.session_state.get('pacote_status_placeholder')
    if placeholder and placeholder != "fallback":
        try: placeholder.empty()
        except Exception as e: print(f"Erro ao limpar placeholder no início de handle_selecao_cliente: {e}", file=sys.stderr)
...
def render_backoffice_clinica():
    ...
    if active_tab == "🗓️ Agenda e Agendamento":
        ...
        # Se não estiver confirmando, mostra o formulário de agendamento
            ...
            # [AJUSTE PARTE 1] Adicionado on_change para sincronizar datas
            form_cols[1].date_input(
                "Data:",
                key="form_data_selecionada",
                min_value=date.today(),
                on_change=lambda: setattr(st.session_state, 'filter_data_selecionada', st.session_state.form_data_selecionada)
            )
            ...
            if servico_data:
                ...
                if tipo_servico == 'Em Grupo':
                    turmas_disponiveis = gerar_turmas_disponiveis(
                        clinic_id,
                        st.session_state.form_data_selecionada, # Usa a data do formulário
        ...
        with view_tab1:
            ...
            st.date_input(
                "Filtrar por data:",
                key='filter_data_selecionada',
                format="DD/MM/YYYY",
                on_change=lambda: setattr(st.session_state, 'form_data_selecionada', st.session_state.filter_data_selecionada)
            )
 
            # Busca agendamentos confirmados para a data selecionada
            agenda_do_dia = buscar_agendamentos_por_data(clinic_id, st.session_state.filter_data_selecionada) # Usa a data do filtro
